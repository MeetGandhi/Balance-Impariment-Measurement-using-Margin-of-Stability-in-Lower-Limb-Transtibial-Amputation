...
for aa=1:qqq
    ...
    for a=1:9
        if a==1 || a==2 || a==9
            ...
            xa=[];
            ya=[];
            if yqq_RHEE_Z(aa+1,xpp(aa,a)+1)<=RHEEmax %&& yqq_RHEE_Z(aa+1,xpp(aa,a)+1)>=RHEEmin
                xa=[xa,yqq_RHEE_X(aa+1,xpp(aa,a)+1)];
                ya=[ya,yqq_RHEE_Y(aa+1,xpp(aa,a)+1)];
            end
            if yqq_LHEE_Z(aa+1,xpp(aa,a)+1)<=LHEEmax %&& yqq_LHEE_Z(aa+1,xpp(aa,a)+1)>=LHEEmin
                xa=[xa,yqq_LHEE_X(aa+1,xpp(aa,a)+1)];
                ya=[ya,yqq_LHEE_Y(aa+1,xpp(aa,a)+1)];
            end
            
            if yqq_RANK_Z(aa+1,xpp(aa,a)+1)<=RANKmax %&& yqq_RANK_Z(aa+1,xpp(aa,a)+1)>=RANKmin
                xa=[xa,yqq_RANK_X(aa+1,xpp(aa,a)+1)];
                ya=[ya,yqq_RANK_Y(aa+1,xpp(aa,a)+1)];
            end
            if yqq_LANK_Z(aa+1,xpp(aa,a)+1)<=LANKmax %&& yqq_LANK_Z(aa+1,xpp(aa,a)+1)>=LANKmin
                xa=[xa,yqq_LANK_X(aa+1,xpp(aa,a)+1)];
                ya=[ya,yqq_LANK_Y(aa+1,xpp(aa,a)+1)];
            end
            
            if yqq_RTOE_Z(aa+1,xpp(aa,a)+1)<=RTOEmax %&& yqq_RTOE_Z(aa+1,xpp(aa,a)+1)>=RTOEmin
                xa=[xa,yqq_RTOE_X(aa+1,xpp(aa,a)+1)];
                ya=[ya,yqq_RTOE_Y(aa+1,xpp(aa,a)+1)];
            end
            if yqq_LTOE_Z(aa+1,xpp(aa,a)+1)<=LTOEmax %&& yqq_LTOE_Z(aa+1,xpp(aa,a)+1)>=LTOEmin
        ...
        if a==3 || a==4
            ...
            xaax=[yqq_RHEE_X(aa+1,xpp(aa,a)+1),yqq_RTOE_X(aa+1,xpp(aa,a)+1),yqq_RANK_X(aa+1,xpp(aa,a)+1)];
            yaay=[yqq_RHEE_Y(aa+1,xpp(aa,a)+1),yqq_RTOE_Y(aa+1,xpp(aa,a)+1),yqq_RANK_Y(aa+1,xpp(aa,a)+1)];
            CVX=[CVX,yqq_COMV_X(aa+1,xpp(aa,a)+1)];
            CVY=[CVY,yqq_COMV_Y(aa+1,xpp(aa,a)+1)];
            FF=[FF,xqq(aa+1,xpp(aa,a)+1)];
            if length(xa)==length(ya) && length(xa)>=3 && length(ya)>=3
        ...
        if a==7 || a==8
            ...
            xaax=[yqq_LHEE_X(aa+1,xpp(aa,a)+1),yqq_LTOE_X(aa+1,xpp(aa,a)+1),yqq_LANK_X(aa+1,xpp(aa,a)+1)];
            yaay=[yqq_LHEE_Y(aa+1,xpp(aa,a)+1),yqq_LTOE_Y(aa+1,xpp(aa,a)+1),yqq_LANK_Y(aa+1,xpp(aa,a)+1)];
            CVX=[CVX,yqq_COMV_X(aa+1,xpp(aa,a)+1)];
            CVY=[CVY,yqq_COMV_Y(aa+1,xpp(aa,a)+1)];
            FF=[FF,xqq(aa+1,xpp(aa,a)+1)];
            if length(xa)==length(ya) && length(xa)>=3 && length(ya)>=3
        ...
        if length(xa)==length(ya) && length(xa)>=3 && length(ya)>=3
            plot(xa(BA)+((a-1)*T),ya(BA),'r-',xc+((a-1)*T),yc,'g*',xaax+((a-1)*T),yaay,'r*',XC+((a-1)*T),YC,'b*');
            hold on;
        end
    end
    M3=getframe(gcf);
